#!/usr/bin/env python3
"""Generate the 2026-05-12 arXiv daily briefing artifacts from parser outputs."""
...
def render_html(classified: dict, trends: dict, html_insights: dict) -> str:
    ...
    thesis = "오늘은 VLA와 world model이 같은 방향을 가리킵니다. 모델을 더 크게 만드는 이야기보다, retrieval, expert routing, latent transition, physical benchmark처럼 실제 행동 루프에서 어디가 실패하는지 나눠 보려는 논문이 훨씬 두껍게 나왔습니다."
    trend_text = f"오늘 /new는 cs.CV {trends['daily_new_counts']['cv']}건, cs.RO {trends['daily_new_counts']['ro']}건이고 dedupe 후 {classified['total']}건 중 {classified['selected']}건이 ROI 버킷에 걸렸습니다. 가장 큰 버킷은 {top[0][0]} {top[0][1]}편, {top[1][0]} {top[1][1]}편, {top[2][0]} {top[2][1]}편입니다. 단순 생성 논문이 많은 날이지만, 제목과 초록을 같이 보면 핵심은 controllable generation 자체보다 world model과 closed-loop 평가가 어디까지 버틸 수 있는지를 묻는 쪽입니다."
...
<div><strong>프롬프트:</strong> prompts/instruction_v20260510.md 기준</div></div>
...
def build() -> None:
    ...
    insights = {
        "date": DATE,
        "daily_thesis": "VLA와 world model이 내부 구조, 물리 일관성, closed-loop 평가를 중심으로 다시 묶이는 날입니다.",
        "clusters": [],
        "research_topics": [
            ...
            {"title": "VLM self-verification stress set", "claim": "의료 VQA와 driving VLM에서 self-verification이 언제 착각을 강화하는지 counterfactual case로 모아야 합니다."},
        ],
        "must_read": [],
        "phylogeny_tags": [],
    }
