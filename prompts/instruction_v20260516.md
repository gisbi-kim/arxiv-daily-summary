# arXiv Daily Briefing — Prompt (sanitized, v20260516)
...
### 8.5.3.c Geometry / SLAM / Reconstruction Watch Lens

`3D/Scene` 버킷은 단순 보관용으로 두면 안 된다. 매일 클러스터 표를 만들기 전에 `3D/Scene` 논문을 별도로 한 번 다시 훑고, classic SLAM 제목이 줄어들어도 아래 신호가 2편 이상 있으면 독립 클러스터 후보로 승격한다.
...
- `Lab action`은 "논문 추적"이 아니라 바로 실행 가능한 형태로 쓴다. 예: `3DGS map / point cloud map / feature-field map을 visual localization 성공률, update cost, dynamic-object failure로 비교`, `VGGT류 feed-forward recon과 COLMAP/MVS baseline을 sparse-view, latency, scale drift로 비교`.
